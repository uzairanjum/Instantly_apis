    ...
    def four_questions_generator(self, request: QuestionGeneratorRequest) -> Union[PackbackCourseQuestionsResponse, None]:
        try:
            if request.course_name and request.course_description:
                ...
                return PackbackCourseQuestionsResponse(course_name=request.course_name, course_description=request.course_description, questions=questions_response.questions, total_completion_tokens=completion_tokens, total_prompt_tokens=prompt_tokens, open_ai_model=request.open_ai_model, token_cost=cost)
